...
public class Main {
    public static void main(String[] args) throws IOException, InterruptedException {
        ...
        Document getCall = data.connection().url("http://119.160.219.22/hlpdsk_pcc/helpdesk/report_by_supplier2.php").data("id_project","ALL").
                data("type_service","ALL").data("date_start", "26" + "-" + mount + "-" + year).data("date_end",lastDate + "-" + lastMount+ "-" + year).data("start","0").
        ...
        if(post > oldCall) {
            ...
            Document getCallCompare = data.connection().url("http://119.160.219.22/hlpdsk_pcc/helpdesk/report_by_supplier2.php").data("id_project","ALL").
                    data("type_service","ALL").data("date_start", "26" + "-" + mount + "-" + year).data("date_end",lastDate + "-" + lastMount+ "-" + year).data("start",compare).
                    data("submit","Find").data("User-Agent"," Mozilla/5.0 (Macintosh").cookie("PHPSESSID",cookie).timeout(6000).post();
            ...
            for(int i=0;i < (post-oldCall);i++) {
                ...
                Jsoup.connect("https://rdcall-production.up.railway.app/post").data(" ",sendData).data("locaton",location).ignoreContentType(true).post();
                Thread.sleep(3000);
            }
